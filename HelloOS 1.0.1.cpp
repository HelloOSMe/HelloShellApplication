...
**/
/**##*声明*##
本程序代码为本人一人所敲，版权所有，请勿转载！
程序代码有上千行，编译的计算机内存必须大于4GB，否则死机后果自负。
程序过大，编译较慢，请耐心等待！
本人不承担任何由程序编译或使用不当而造成的死机！
**/
/**   #System#
----[]        []
    []        []
...
*/
/**更新记录:
 * 2022/04/24: (N)开始项目(@begintime),更新开始界面
 * 2022/05/02: (N)更新欢迎界面,建立磁盘界面,运行界面和7个命令
 * 2022/05/03: (N)更新8个命令,系统完善
 * 2022/05/04: (N)更新chgpass和run命令,使应用无法在不对应文件夹打开,有了管理员
 * 2022/05/06: (N)修理chgpass命令的bug
 * 2022/05/07: (N)修理chguser命令的bug及kill、open命令无法使用的bug,重启后自动回归H盘
 * 2022/05/14: (N)更新tree,cop命令
 * 2022/05/19: (N)更新logoff命令与操作界面优化
 * 2022/05/21: (N)更新HelloStore应用
 * 2022/05/25: (S)525更新_优化开机动画
 * 2022/05/27: (S)"527省赛决赛特别版",更改磁盘建立界面,允许命令输入大写字母
 * 2022/06/01:1(W)紧急更新: 修复Linux服务器clear命令缺陷
              2(N)更新版本检测,time命令
 * 2022/06/06: (N)更新color命令
 * 2022/06/18: (S)"考试加油"更新,增加密码认证。
 * 2022/06/19: (N)更新BASIC命令 
 * 2022/06/29: (N)更新time命令,添加新管理员(测试)用户
 * 2022/06/30: (N)更新输入命令界面,修理新管理员密码bug
 * 2022/07/13: (N)更新盘符，可进入系统文件夹。
*/
/***============代码正式开始=============***/
#ifndef __HelloOS
#define __HelloOS
...
/**
---------------------后记---------------------
|    HelloOS，是我有史以来编写的最大的项目。 |
|从只有开始界面的v0.0.1到完善版v1.0.6，花费了|
|我大约2个月的时间。而目前HelloOS还是一个应用|
|程序，只是一个"Windows和Linux的玩物"。      |
|    这个项目确实不好写，我遇到的首要问题就是|
|文件操作后无法输出到终端，其二就是颜色问题：|
|这个系统必须兼用Linux和Windows(我要用IOS，而|
|编写C++只可以用IOS在Linux网上编)，Linux的\03|
|3和WindowsAPI的SetConsoleTextAttribute不同，|
|前者可以写为cout<<"\033[xx;ym"，而后者需写一|
|大堆函数。这可就不好弄了，我只能暂停Windows |
|的color程序(只能在DOS或BASIC里用color命令)。|
|当然，现在我弄好了。                        |
|    目前，这个系统还不算非常完善，毕竟它只有|
|1000行代码。不过，我期待与众程序员一起，创建|
|更好的系统!!!                               |
|                              by Ziming Zhou|
|                                  2022/06/29|
|                                            |
...
----------------------------------------------
可运行的系统:Windows 95-Windows 11,Linux各版本
**/
/**
#特别鸣谢#
金熙然先生:对盘符的建议
